...
^https?://mp\.weixin\.qq\.com/s.+?k=feizao url response-body var ua = navigator.userAgent; response-body var ua = navigator.userAgent; setTimeout(()=>window.history.back(),2000);
  ...
  if (typeof $response !== "undefined") {
    if ($request.url.match(/https?:\/\/mp\.weixin\.qq\.com\/s.+?&k=feizao/)) {
      ...
      if (body.indexOf('var ua = navigator.userAgent;') > 0) {
        body = body.replace('var ua = navigator.userAgent;', 'var ua = navigator.userAgent; setTimeout(()=>window.history.back(),2000);')
        $.done({body})
      } else {
        $.log(`注入失败：未找到替换数据`)
      }
