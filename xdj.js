...
微信公众号柠檬玩机交流
下载地址：
http://f8sy.xyz/28611506
http://kemh.xyz/28611506
http://od5l.xyz/28611506
Tg群 t.me/ningmengwj
使用方法
获取数据随便进入一个直播间一下
定时静默运行2分钟一次获取该主播最新直播源
自行到圈叉查看日志复制到播放器播放
...
function GetCookie() {
if($request&&$request.url.indexOf("OpenAPI/v1/private/getPrivateLimit")>=0) {
...
if(xnmurl)    $.setdata(xnmurl,`xnmurl${status}`)
    ...
    $.log(`[${xnm}] 获取柠檬小奶猫请求: 成功,xnmheader: ${xnmheader}`)
$.log(`[${xnm}] 获取柠檬小奶猫: 成功,xnmheader: ${xnmurl}`)
    $.msg(`柠檬小奶猫${status}: 成功🎉`, ``)
}
}
...
async function nmget(){
 return new Promise((resolve) => {
    fj = xnmurl.match(/uid=(\d+)/)[1]
    let plant6_url = {
   		url: `http://47.242.106.199/nmget.php?uid=`+fj,
   ...
   $.post(plant6_url,async(error, response, data) =>{
    ...
    async function nmurl(){
 return new Promise((resolve) => {
    let havest_url = {
   		url: `http://47.242.106.199/nmurl.php?data=`+sj,
        //headers: JSON.parse(kxdfsheader),
   ...
   $.post(havest_url,async(error, response, data) =>{
    try{
        ...
        if(result.code == 0)
          //$.log(data)
 $.log("主播:"+result.data.anchor.nickname+"\n直播流地址:\n"+result.data.stream.pull_url)
        if(data == -1)
          $.log(data)
        }catch(e) {
          $.logErr(e, response);
